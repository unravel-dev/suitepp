...
#define SUITEPP_EXPECT_THROWS_AS(expr, exception_type)                                                       \
	suite::check(                                                                                            \
		...
		{                                                                                                    \
			...
			{                                                                                                \
				return suite::result{                                                                        \
					false, std::string(#expr " expected to throw " #exception_type " but threw \"") +        \
							   e.what() + "\""};                                                             \
			}                                                                                                \
			catch(...)                                                                                       \
			{                                                                                                \
...
#define SUITEPP_EXPECT_NOTHROWS(expr)                                                                        \
	suite::check(#expr, __FILE__, __LINE__,                                                                  \
				 ...
				 {                                                                                           \
					 ...
					 {                                                                                       \
						 return suite::result{                                                               \
							 false, std::string(#expr " expected NOT to throw but did : ") + e.what()};      \
					 }                                                                                       \
					 catch(...)                                                                              \
					 {                                                                                       \
...
#define REQUIRE_THROWS_AS(expr, exception_type)                                                              \
	SUITEPP_EXPECT_THROWS_AS(expr, exception_type).skip_report_on_success(true).exit_on_fail(true).run()
#define REQUIRE_NOTHROWS(expr)                                                                               \
	SUITEPP_EXPECT_NOTHROWS(expr).skip_report_on_success(true).exit_on_fail(true).run()
#define SCENARIO(name)                                                                                       \
	suite::scenario("Scenario: ", name, __FILE__, __LINE__).exit_on_fail(true).mutate() = [&]
...
template <typename T>
using has_std_to_string = is_detected<std_to_string_expression, T>;

// 2 - detecting if to_string is valid on T
template <typename T>
using to_string_expression = decltype(to_string(std::declval<T>()));
template <typename T>
using has_to_string = is_detected<to_string_expression, T>;
...
template <typename T>
using has_ostringstream = is_detected<ostringstream_expression, T>;
...
struct entry
{
	...
	entry(const std::string& text, const char* const file, int32_t line)
		: text_(text)
		, file_(file)
		, line_(line)
	{
...
{
public:
	check(const std::string& text, const char* const file, int32_t line, std::function<result()> f)
...
inline bool match_label(const std::string& label_)
{
	...
	if(label_matcher.front() == '*') // Match it as suffix.
	{
		return label_size > label_matcher_size &&
			   label_.find(label_matcher.c_str() + 1, label_size - label_matcher_size + 1) !=
				   std::string::npos;
	}
...
{
	basic_test(const std::string& text, const char* const file, int32_t line)
		: entry(text, file, line)
	{
	}
...
{
	scenario(const std::string& header, const std::string& text, const char* const file, int32_t line)
...
{
	scenario_group(const std::string& header, const std::string& text, const char* const file, int32_t line)
		: basic_test(std::string(header).append(text), file, line)
	{
		scenario_stack().emplace_back(text);
	}
...
{
	scenario_segment(const std::string& header, const std::string& text, const char* const file, int32_t line)
		: basic_test(std::string(header).append(text), file, line)
	{
		++current_indent();
	}
